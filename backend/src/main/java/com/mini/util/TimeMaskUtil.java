...
public class TimeMaskUtil {
    ...
    // 디폴트 시간표
    // 30분 단위로 24시간을 쪼개서 비트마스크를 생성
    public static long generateTimeMask(String times) {
        if (times == null || times.length() != 48) {
            throw new IllegalArgumentException("비트마스크는 정확히 48개의 값이어야 합니다.");
        }
        ...
        for (int i = 0; i < 48; i++) {
            ...
            if (c != '0' && c != '1') {
                throw new IllegalArgumentException("비트마스크는 '0'과 '1'만 포함해야 합니다.");
            }
            if (c == '1') {
                timeMask |= (1L << (47 - i));
            }
